...
int main (int argc, char** argv) 
{
    ...
    powerups.push_back (new FlashyPowerup ( 75, 150, RADIUS));
    powerups.push_back (new Wormhole      (250, 150, RADIUS));
    powerups.push_back (new MegaPowerup   (425, 150, RADIUS));
    ...
    while (SSDL_IsNextFrame ())
    {
        ...
        for (unsigned int i = 0; i < powerups.size(); ++i)
            (*powerups[i]).animate();//<-- Change this to animate
